...
### 문제1) 단지에 있는 동의 폴리곤 데이터를 이용하여 런타임에 메쉬를 구성하여 렌더링해주세요.(4점)
...
### 문제2) 문제1의 결과를 바탕으로 생성된 메쉬에 텍스쳐를 매핑합니다.(6점)
...
1번(앞) : 정점의 normal과 Vector3.forward의 y축 각도가 180 <= n && n <= 220 에 해당하는 경우

2번 : 1, 3을 제외한 모든 경우

3번(위, 아래) : 정점의 normal이 Vector3.up || Vector3.down과 일치하는 경우
...
1. <b>아파트 단지에 텍스쳐 맵핑을 적용 (1점)

2. 주어진 텍스쳐 이미지를 문제에서 제시한 영역만큼 구분하여 적용 (2점)

3. 적용된 텍스쳐를 문제에 제시한 방향 조건에 맞추어 적용 (3점)</b>

### 코드 설계시 우대되는 부분

유니티 라이프 사이클 함수 (Awake, Start, OnEnable, OnDisable, OnDestroy, Update) 이외의 직접 라이플 사이클을 구현 <b>(점수 획득한 문제에 가산점 +2점)</b>
